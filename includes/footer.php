            </main>
            <footer class="mt-auto bg-white border-top py-3 px-4">
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-2">
                    <?php require_once __DIR__ . '/app_helper.php'; ?>
                    <p class="text-muted small mb-0">&copy; <?= date('Y') ?> <span class="fw-bold text-primary"><?= htmlspecialchars(app_name()) ?></span>. All rights reserved.</p>
                    <div class="d-flex gap-3">
                        <a href="#" class="text-muted text-decoration-none small">Support</a>
                        <a href="#" class="text-muted text-decoration-none small">Privacy Policy</a>
                        <a href="#" class="text-muted text-decoration-none small">Terms</a>
                    </div>
                </div>
            </footer>
        </div> <!-- End page-content-wrapper -->
    </div> <!-- End d-flex wrapper -->

    <!-- Bootstrap 5 JS Bundle with Popper -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
    <!-- Custom JS -->
    <script src="../assets/js/script.js"></script>
</body>
</html>